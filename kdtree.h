...
class KDTree
{
  ...
  private:
     ...
     */
    void construct(int start, int end, int dimension, KDTreeNode *& subRoot);

    void quickSelect(int start, int end, int k, int dimension);

    int partition(int start, int end, int dimension);

    KDTreeNode *copyHelper(const KDTreeNode *& node);

    void clearHelper(KDTreeNode *& root);

    Point<Dim> findHelper(KDTreeNode * r, int d, const Point<Dim>& target) const;
};
